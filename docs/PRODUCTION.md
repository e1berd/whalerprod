...
Если DNS зоны управляется Beget, используйте встроенный production-путь через
`lego`, у которого есть DNS provider `beget`:

```dotenv
BEGET_USERNAME=your-beget-login
BEGET_PASSWORD=your-beget-api-password
STAND_TLS_DIRECTIVE="tls /lego-data/certificates/_.stand.example.com.crt /lego-data/certificates/_.stand.example.com.key"
```

Затем выпустите wildcard-сертификат:

```bash
just beget-stand-cert
```

Сертификаты сохраняются в `storage/lego`, монтируются в Caddy как `/lego-data`, а
`STAND_TLS_DIRECTIVE` подключает их только для `*.stand.example.com`. Для
продления используйте:

```bash
just beget-stand-cert-renew
```
